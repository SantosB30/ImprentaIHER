...
@If Session("mensaje") <> Nothing Then
    If Session("mensaje").ToString().Equals("Usuario eliminado") Then
        @<script>
    ...
    ElseIf Session("mensaje").ToString().Contains("Password reestablecida") Then
        @<script>
             window.onload = function () {
                 swal({
                     title: "¡Confirmación!",
                     text: "¡Se reestableció la contraseña!",
                     type: "success"
                 });
             };
        </script>
